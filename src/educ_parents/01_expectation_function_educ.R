...
#' @param outcome_name the name of the outcome column, for example wealth_perc
...
#' @param educ_grp parental wealth percentile group: "low", "mid", "high" or "all"
...
kansenkaart_expect <- function(cohort_dat, outcome_name, 
                               ...
                               educ_grp = "all", migration_grp = "all", 
  ...
  ## education group ----
  if (educ_grp != "all") {
    cohort_dat <- cohort_dat %>% filter(parents_education == educ_grp)
  }
  ...
  # obtain confidence intervals from the regression
  conf <- confint(fit, level = conf_level)
  
  # put together in one vector
  pred <- data.frame(intercept, conf[1, 1], conf[1, 2])
